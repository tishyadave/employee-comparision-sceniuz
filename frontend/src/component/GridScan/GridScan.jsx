...
export const GridScan = ({
  ...
  const [faceapiInstance, setFaceapiInstance] = useState(null);
  ...
  useEffect(() => {
    ...
    const start = async () => {
      if (!enableWebcam || !modelsReady || !faceapiInstance) return;
      if (!video) return;
      ...
      const opts = new faceapiInstance.TinyFaceDetectorOptions({ inputSize: 320, scoreThreshold: 0.5 });
      ...
      const detect = async ts => {
        ...
        if (ts - lastDetect >= 33) {
          lastDetect = ts;
          try {
            const res = await faceapiInstance.detectSingleFace(video, opts).withFaceLandmarks(true);
  ...
  }, [enableWebcam, modelsReady, faceapiInstance, depthResponse]);
