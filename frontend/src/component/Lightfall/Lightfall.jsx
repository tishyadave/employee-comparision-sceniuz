...
const Lightfall = ({
  ...
  const mouseConfigRef = useRef({ mouseInteraction, mouseDampening });

  useEffect(() => {
    mouseConfigRef.current = { mouseInteraction, mouseDampening };
  }, [mouseInteraction, mouseDampening]);
  ...
  useEffect(() => {
    ...
    const onPointerMove = e => {
      if (!mouseConfigRef.current.mouseInteraction) return;
      const rect = canvas.getBoundingClientRect();
      const scale = renderer.dpr || 1;
      const x = (e.clientX - rect.left) * scale;
      const y = (rect.height - (e.clientY - rect.top)) * scale;
      mouseTargetRef.current = [x, y];
      if (mouseConfigRef.current.mouseDampening <= 0) {
    ...
    const loop = t => {
      ...
      const dampening = mouseConfigRef.current.mouseDampening;
      if (dampening > 0) {
